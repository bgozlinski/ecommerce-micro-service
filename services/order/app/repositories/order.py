...
def create_order_from_cart(db: Session, user_id: int):
    ...
    for ci in cart_items:
        ...
        resp = requests.post(f"{settings.INVENTORY_SERVICE_URL}/api/v1/inventory/reserve",
    ...
    db.commit()
    # TODO: Publish event order_created
    return order
...
def update_order_status(db: Session, order_id: int, status: str):
    ...
    if status == "paid":
        ...
        # TODO: Publish event order_paid
    elif status == "failed":
        # TODO: Publish event order_failed
        pass
...
def cancel_order(db: Session, order_id: int, user_id: int):
    ...
    db.commit()
    # TODO: Publish event order_cancelled
    return True